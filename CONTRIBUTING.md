...
- Each PR references and closes a single issue (`Closes #N`, `Fixes #N`, or `Resolves #N`). Use `Refs #N` only when the PR is one of several closing a larger Epic.
...
```bash
pnpm test       # Vitest unit tests — should be 100% pass
pnpm ts-check   # TypeScript type check — should be clean
pnpm lint       # ESLint — should not introduce new errors
```
...
- 🐛 [Bug report](./issues/new) — when something doesn't work as expected
- 💡 [Feature request](./issues/new) — when you want to propose new functionality
