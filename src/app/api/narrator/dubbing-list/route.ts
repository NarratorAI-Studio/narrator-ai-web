...
async function fetchDubbingPage(
  appKey: string,
  page: number,
  size: number
): Promise<NarratorMetadataListPayload> {
  const payload = unwrapMetadataListPayload(
    await fetchNarratorMetadata(appKey, '/narrator/dubbing-list', {
      page: String(page),
      size: String(size),
    })
  );
  return {
    ...
    items: Array.isArray(payload.items)
      ? filterMetadataPlaceholderItems(payload.items, 'dubbing_id')
      : [],
  };
}
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (sp.has('page') || sp.has('size')) {
      return NextResponse.json({
        success: true,
        data: await fetchDubbingPage(appKey, page, size),
      });
    }

    const first = await fetchDubbingPage(appKey, DEFAULT_PAGE, DEFAULT_SIZE);
    ...
    if (total && mergedItems.length < total) {
      for (let nextPage = DEFAULT_PAGE + 1; nextPage <= MAX_AUTO_PAGES; nextPage += 1) {
        const next = await fetchDubbingPage(appKey, nextPage, DEFAULT_SIZE);
    ...
    return NextResponse.json(
      { success: false, error: (e as Error).message || '获取配音列表失败' },
      { status: 500 }
    );
  }
}
