...
/**
 * GET /api/narrator/query-subtitle-removal/[taskId] - 查询字幕擦除任务状态
 */
export async function GET(
  ...
  try {
    ...
    const result = (await callNarratorProxyGet(
      appKey,
      `/narrator/subtitle-removal/query/${encodeURIComponent(taskId)}`
    )) as { data?: unknown };
    ...
    return NextResponse.json(
      { success: false, error: (e as Error).message || '查询字幕擦除任务失败' },
      { status: 500 }
    );
  }
}
