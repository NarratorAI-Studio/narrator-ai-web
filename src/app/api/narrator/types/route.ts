...
/**
 * GET /api/narrator/types — v1 解说类型列表
 *
 * Routed through backend `/narrator/types` per regression coverage.
 */
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      { success: false, error: (e as Error).message || 'Failed to fetch narrator types' },
      { status: 500 }
    );
  }
}
