...
import {
  ArrowLeft, FileText, Search, FolderSearch, Loader2, CheckCircle2,
...
const LANGUAGES = [
  'Auto-Detect','Chinese','English','Japanese','Korean','French','German',
  'Spanish','Italian','Portuguese','Russian','Arabic','Thai','Vietnamese',
  'Indonesian','Malay','Turkish','Polish','Dutch','Swedish','Danish',
];
...
const MODES = [
  { value: 1, label: '本地提取', desc: '速度最快，适合硬编码字幕' },
  { value: 2, label: '视觉提取', desc: '视觉识别，适合复杂场景字幕' },
  { value: 3, label: '音频提取', desc: '音频转写，适合无内嵌字幕' },
];

export default function SubtitleExtractPage() {
  ...
  const [ignoreSoftCoded, setIgnoreSoftCoded] = useState(false);
  const [yOffsetStart, setYOffsetStart] = useState<string>('0');
  const [yOffsetEnd, setYOffsetEnd] = useState<string>('1');
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload: Record<string, unknown> = { file_id: validIds, mode };
      if (mode !== 1) {
        payload.language = language || 'Auto-Detect';
        if (subtitlePosition) payload.subtitle_position = subtitlePosition;
        payload.ignore_soft_coded = ignoreSoftCoded;
        if (yOffsetStart !== '') payload.y_offset_start = Number(yOffsetStart);
        if (yOffsetEnd !== '') payload.y_offset_end = Number(yOffsetEnd);
        if (timeStart !== '') payload.time_start = Number(timeStart);
        if (timeEnd !== '') payload.time_end = Number(timeEnd);
      }

      const r = await fetch('/api/tools/subtitle-extract', {
        method: 'POST', headers: apiHeaders(), body: JSON.stringify(payload),
      });
      const j = await r.json() as ToolResponse;
      if (j.success) { setResult(j.data ?? {}); showToast('success', '字幕提取任务已创建！'); }
  ...
  if (!SUBTITLE_TOOLS_ENABLED) {
    return (
      <SubtitleToolsUnavailable
        title="字幕提取"
        description="字幕提取工具正在接入 Web 任务记录和扣费链路，当前不支持从独立工具页提交任务。"
      />
    );
  }
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
        ...
        <div className="container mx-auto px-4 py-3 flex items-center justify-between h-14">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg font-semibold flex items-center gap-2">
              <FileText className="h-5 w-5 text-primary" />字幕提取
            </h1>
          </div>
          {!appKey && (
            <AppNavLink href="/">
      ...
      <main className="container mx-auto px-4 py-8 max-w-2xl space-y-6">
        {/* Description */}
        <div>
          <h2 className="text-2xl font-bold mb-1">字幕提取</h2>
          <p className="text-muted-foreground text-sm">从视频文件中提取字幕 SRT，支持本地、视觉和音频三种提取模式。</p>
        </div>

        {/* Form */}
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-base">创建提取任务</CardTitle>
          ...
          <CardContent className="space-y-5">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">输入文件 ID 或从云盘选择</p>
              {fileIds.map((id, idx) => (
                <div key={idx} className="flex gap-2">
                  <Input
                    placeholder="文件 ID（如 file_abc123...）"
                    value={id}
            ...
            {/* Mode */}
            <div className="space-y-2">
              <Label className="text-sm font-medium">提取模式 <span className="text-red-500">*</span></Label>
              <div className="grid grid-cols-3 gap-2">
                {MODES.map(m => (
                  <button key={m.value} type="button" onClick={() => setMode(m.value)}
                    className={`rounded-xl border p-3 text-left transition-all ${mode === m.value ? 'border-primary bg-primary/10' : 'border-border hover:border-primary/40'}`}>
                    <p className={`text-sm font-medium ${mode === m.value ? 'text-primary' : ''}`}>{m.label}</p>
            ...
            {/* Language / Position / Advanced — 本地提取模式(mode=1)不需要这些参数 */}
            {mode !== 1 && (
              <>
                {/* Language */}
                ...
                <div className="space-y-1.5">
                  ...
                  <Select value={subtitlePosition} onValueChange={setSubtitlePosition}>
                    <SelectTrigger className="h-9 text-sm"><SelectValue /></SelectTrigger>
                    <SelectContent>
                      {SUBTITLE_POSITIONS.map(p => <SelectItem key={p.value} value={p.value}>{p.label}</SelectItem>)}
                    </SelectContent>
                  </Select>
                </div>
                ...
                <details className="group">
                  ...
                  <div className="mt-3 space-y-4 pl-4 border-l">
                    {/* Ignore soft coded */}
                    <div className="flex items-center gap-3">
                      <button type="button"
                        onClick={() => setIgnoreSoftCoded(v => !v)}
                        className={`w-10 h-5 rounded-full transition-colors relative ${ignoreSoftCoded ? 'bg-primary' : 'bg-muted border'}`}>
                        <span className={`absolute top-0.5 h-4 w-4 rounded-full bg-white shadow transition-all ${ignoreSoftCoded ? 'left-5' : 'left-0.5'}`} />
                      </button>
                      <Label className="text-sm cursor-pointer" onClick={() => setIgnoreSoftCoded(v => !v)}>
                        忽略软编码字幕
                      </Label>
                    </div>
                    {/* Y offset */}
                    <div className="grid grid-cols-2 gap-3">
                      <div className="space-y-1.5">
                        <Label className="text-xs">垂直偏移开始 (0~1)</Label>
                        <Input type="number" min={0} max={1} step={0.01} value={yOffsetStart}
                          onChange={e => setYOffsetStart(e.target.value)} className="h-8 text-sm" />
                      </div>
                      <div className="space-y-1.5">
                        <Label className="text-xs">垂直偏移结束 (0~1)</Label>
                        <Input type="number" min={0} max={1} step={0.01} value={yOffsetEnd}
                          onChange={e => setYOffsetEnd(e.target.value)} className="h-8 text-sm" />
                      </div>
                    </div>
                    {/* Time range */}
                    <div className="grid grid-cols-2 gap-3">
                      <div className="space-y-1.5">
                        <Label className="text-xs">开始时间（秒，留空忽略）</Label>
                        <Input type="number" min={0} value={timeStart}
                          onChange={e => setTimeStart(e.target.value)} placeholder="0" className="h-8 text-sm" />
                      </div>
                      <div className="space-y-1.5">
                        <Label className="text-xs">结束时间（秒，留空忽略）</Label>
                        <Input type="number" min={0} value={timeEnd}
                          onChange={e => setTimeEnd(e.target.value)} placeholder="0" className="h-8 text-sm" />
                      </div>
                    </div>
                  </div>
                </details>
